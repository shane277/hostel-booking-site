-- Run this script in your Supabase SQL Editor to set up the Roommate Chat system
...
DROP POLICY IF EXISTS "Users can view room chats for their booked rooms" ON public.room_chats;
DROP POLICY IF EXISTS "Users can send messages to their booked rooms" ON public.room_chats;
DROP POLICY IF EXISTS "Users can update their own messages" ON public.room_chats;
DROP POLICY IF EXISTS "Users can delete their own messages" ON public.room_chats;
...
DROP FUNCTION IF EXISTS get_room_chat_participants(uuid);
DROP FUNCTION IF EXISTS get_user_room_chats(uuid);
...
-- Success message
SELECT 'Roommate Chat system has been successfully set up! 🎉' as status;
