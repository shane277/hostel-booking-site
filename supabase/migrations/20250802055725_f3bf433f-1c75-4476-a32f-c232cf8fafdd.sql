...
-- Add rooms to realtime publication
...
-- Create function to check room availability before booking
...
BEGIN
  ...
  DECLARE room_uuid uuid := COALESCE(NEW.room_id, OLD.room_id);
...
CREATE TRIGGER update_room_availability_on_booking_change
  AFTER INSERT OR UPDATE OR DELETE ON public.bookings
  FOR EACH ROW
  EXECUTE FUNCTION public.update_room_availability();

-- Create function to handle booking conflicts
