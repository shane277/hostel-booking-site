...
serve(async (req) => {
  ...
  try {
    ...
    const { bookingId, amount, hostelName, roomNumber } = await req.json();
    if (!bookingId || !amount) throw new Error("Missing required fields");
    logStep("Request data received", { bookingId, amount, hostelName, roomNumber });
      ...
      line_items: [
        {
          price_data: {
            currency: "usd",
            product_data: { 
              name: `${hostelName} - Room ${roomNumber}`,
